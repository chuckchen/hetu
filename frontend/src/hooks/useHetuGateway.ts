...
import { CONTRACTS, HETU_GATEWAY_ABI, WRAPPER_ABI } from '../lib/contracts';
...
const GATEWAY_ABI = HETU_GATEWAY_ABI;
...
export function useHetuGateway() {
  ...
  const { data: paymentCount } = useReadContract({
    address: CONTRACTS.HETU_GATEWAY,
    abi: GATEWAY_ABI,
  ...
  const { data: isOperator, refetch: refetchOperator } = useReadContract({
    ...
    args: address ? [address, CONTRACTS.HETU_GATEWAY] : undefined,
    query: { enabled: !!address },
  });
  ...
  const approveGateway = useCallback(async (): Promise<`0x${string}` | null> => {
    if (!address || !isConnected) {
      console.error('[Hetu] Not connected');
      return null;
    }

    setIsProcessing(true);
    try {
      console.log('[Hetu] Approving gateway as operator...');
      const hash = await writeContractAsync({
        address: CONTRACTS.WRAPPER,
        abi: WRAPPER_ABI_PARSED,
        functionName: 'setOperator',
        args: [CONTRACTS.HETU_GATEWAY, OPERATOR_EXPIRY],
      });
      ...
      console.log('[Hetu] Gateway approved as operator');
      return hash;
    } catch (err) {
      console.error('[Hetu] Approval failed:', err);
      throw err;
    } finally {
  ...
  const send = useCallback(
    async (params: SendParams): Promise<TransactionResult | null> => {
      ...
      if (!address || !isConnected || !fhevmReady) {
        console.error('[Hetu] Not ready for payment');
        return null;
      }
      ...
      try {
        ...
        if (!isOperator) {
          console.log('[Hetu] Gateway not approved, requesting approval...');
          await approveGateway();
          // Wait for state to propagate on-chain before proceeding
          console.log('[Hetu] Waiting for approval to propagate...');
          await new Promise(resolve => setTimeout(resolve, 2000));
        }

        console.log('[Hetu] Encrypting amount for GATEWAY...');
        ...
        const encrypted = await encryptAmount(amount, CONTRACTS.HETU_GATEWAY);
        if (!encrypted || !encrypted.handles[0]) {
          throw new Error('Failed to encrypt amount');
        }

        console.log('[Hetu] Sending to', recipient);
        const hash = await writeContractAsync({
          address: CONTRACTS.HETU_GATEWAY,
          abi: GATEWAY_ABI,
          functionName: 'send',
        ...
        console.log('[Hetu] Payment sent! ID:', paymentId);
        return { hash, id: paymentId };
      } catch (err) {
        console.error('[Hetu] Send failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const multiSend = useCallback(
    async (params: MultiSendParams): Promise<TransactionResult | null> => {
      ...
      try {
        ...
        if (!isOperator) {
          console.log('[Hetu] Gateway not approved, approving...');
          await approveGateway();
          // Wait for approval to be processed
          await new Promise(resolve => setTimeout(resolve, 2000));
        }

        console.log('[Hetu] Encrypting', amounts.length, 'amounts for GATEWAY...');
        
        // Parallelize encryption for better performance
        const encryptedResults = await Promise.all(
          amounts.map(amount => encryptAmount(amount, CONTRACTS.HETU_GATEWAY))
        );
        ...
        console.log('[Hetu] Multi-sending to', recipients.length, 'recipients');
        const hash = await writeContractAsync({
          address: CONTRACTS.HETU_GATEWAY,
          abi: GATEWAY_ABI,
        ...
        console.log('[Hetu] Multi-send complete!');
        return { hash, id: hash }; // Use tx hash as ID for multi-send
      } catch (err) {
        console.error('[Hetu] Multi-send failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const createRequest = useCallback(
    async (params: RequestParams): Promise<TransactionResult | null> => {
      ...
      try {
        console.log('[Hetu] Creating payment request...');
        const encrypted = await encryptAmount(amount, CONTRACTS.HETU_GATEWAY);
        if (!encrypted || !encrypted.handles[0]) {
          throw new Error('Failed to encrypt amount');
        }
        ...
        console.log('[Hetu] Request created! ID:', requestId);
        return { hash, id: requestId };
      } catch (err) {
        console.error('[Hetu] Create request failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const fulfillRequest = useCallback(
    async (requestId: `0x${string}`, amount: bigint): Promise<TransactionResult | null> => {
      ...
      try {
        ...
        console.log('[Hetu] Fulfilling request', requestId);
        ...
        console.log('[Hetu] Request fulfilled! Payment ID:', paymentId);
        return { hash, id: paymentId };
      } catch (err) {
        console.error('[Hetu] Fulfill request failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const cancelRequest = useCallback(
    async (requestId: `0x${string}`): Promise<`0x${string}` | null> => {
      ...
      try {
        ...
        console.log('[Hetu] Request cancelled');
        return hash;
      } catch (err) {
        console.error('[Hetu] Cancel request failed:', err);
        throw err;
      }
    },
  ...
  const createSubscription = useCallback(
    async (params: SubscriptionParams): Promise<TransactionResult | null> => {
      ...
      try {
        console.log('[Hetu] Creating subscription to', recipient);
        const encrypted = await encryptAmount(amount, CONTRACTS.HETU_GATEWAY);
        if (!encrypted || !encrypted.handles[0]) {
          throw new Error('Failed to encrypt amount');
        }
        ...
        console.log('[Hetu] Subscription created! ID:', subscriptionId);
        return { hash, id: subscriptionId };
      } catch (err) {
        console.error('[Hetu] Create subscription failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const executeSubscription = useCallback(
    async (subscriptionId: `0x${string}`): Promise<TransactionResult | null> => {
      ...
      try {
        ...
        console.log('[Hetu] Executing subscription', subscriptionId);
        const hash = await writeContractAsync({
          address: CONTRACTS.HETU_GATEWAY,
          abi: GATEWAY_ABI,
        ...
        console.log('[Hetu] Subscription payment made! ID:', paymentId);
        return { hash, id: paymentId };
      } catch (err) {
        console.error('[Hetu] Execute subscription failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const cancelSubscription = useCallback(
    async (subscriptionId: `0x${string}`): Promise<`0x${string}` | null> => {
      ...
      try {
        ...
        console.log('[Hetu] Subscription cancelled');
        return hash;
      } catch (err) {
        console.error('[Hetu] Cancel subscription failed:', err);
        throw err;
      }
    },
    [address, isConnected, writeContractAsync, publicClient]
  ...
  const refund = useCallback(
    async (paymentId: `0x${string}`): Promise<`0x${string}` | null> => {
      ...
      try {
        ...
        console.log('[Hetu] Refunding payment', paymentId);
        const hash = await writeContractAsync({
          address: CONTRACTS.HETU_GATEWAY,
          abi: GATEWAY_ABI,
          functionName: 'refund',
        ...
        console.log('[Hetu] Refund processed!');
        return hash;
      } catch (err) {
        console.error('[Hetu] Refund failed:', err);
        throw err;
      } finally {
        setIsProcessing(false);
      }
  ...
  const getPaymentInfo = useCallback(
    async (paymentId: `0x${string}`): Promise<PaymentInfo | null> => {
      ...
      // Validate paymentId is proper bytes32 (66 chars with 0x prefix)
      if (!paymentId || paymentId.length !== 66 || !paymentId.startsWith('0x')) {
        console.warn(`[Hetu] Invalid paymentId format: ${paymentId}`);
        return null;
      }
      ...
      try {
        ...
        console.error('[Hetu] Get payment info failed:', err);
        return null;
      }
    },
    [publicClient]
  ...
  const getRequestInfo = useCallback(
    async (requestId: `0x${string}`): Promise<RequestInfo | null> => {
      ...
      // Validate requestId is proper bytes32 (66 chars with 0x prefix)
      if (!requestId || requestId.length !== 66 || !requestId.startsWith('0x')) {
        console.warn(`[Hetu] Invalid requestId format: ${requestId}`);
        return null;
      }
      ...
      try {
        ...
        console.error('[Hetu] Get request info failed:', err);
        return null;
      }
    },
    [publicClient]
  ...
  const getSubscriptionInfo = useCallback(
    async (subscriptionId: `0x${string}`): Promise<SubscriptionInfo | null> => {
      ...
      // Validate subscriptionId is proper bytes32 (66 chars with 0x prefix)
      if (!subscriptionId || subscriptionId.length !== 66 || !subscriptionId.startsWith('0x')) {
        console.warn(`[Hetu] Invalid subscriptionId format: ${subscriptionId}`);
        return null;
      }
      ...
      try {
        ...
        console.error('[Hetu] Get subscription info failed:', err);
        return null;
      }
    },
    [publicClient]
