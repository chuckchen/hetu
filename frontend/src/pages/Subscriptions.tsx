...
export function Subscriptions() {
  ...
  const { createSubscription, cancelSubscription, executeSubscription, isProcessing, fhevmReady, getSubscriptionInfo } = useHetuGateway();
  ...
  const fetchSubscriptions = useCallback(async () => {
    ...
    try {
      ...
      const stored = localStorage.getItem('hetu_subscriptions');
      if (stored) {
        try {
  ...
  const saveSubscriptions = (subs: Subscription[]) => {
    setSubscriptions(subs);
    localStorage.setItem('hetu_subscriptions', JSON.stringify(subs));
  };
