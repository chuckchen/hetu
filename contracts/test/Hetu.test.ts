...
/**
 * Hetu Protocol Test Suite
 *
 * Contracts:
 * - ConfidentialUSDCWrapper: Wraps USDC → cUSDC (FHE-encrypted)
 * - HetuPaymentGateway: Privacy-first P2P payments
 */
describe("Hetu Protocol", function () {
  let usdc: any;
  let wrapper: any;
  let gateway: any;
  ...
  beforeEach(async function () {
    ...
    wrapper = await Wrapper.deploy(await usdc.getAddress(), "Confidential USDC", "cUSDC", "");
    await wrapper.waitForDeployment();

    // Deploy HetuPaymentGateway
    const Gateway = await ethers.getContractFactory("HetuPaymentGateway");
  ...
  describe("ConfidentialUSDCWrapper", function () {
    ...
    describe("Wrapping USDC → cUSDC", function () {
      ...
      it("should decrypt to correct wrapped amount", async function () {
        ...
        const handle = await wrapper.confidentialBalanceOf(alice.address);
        const decrypted = await fhevm.userDecryptEuint(FhevmType.euint64, handle, await wrapper.getAddress(), alice);
      ...
      it("should allow wrapping to different recipient", async function () {
        ...
        const decrypted = await fhevm.userDecryptEuint(FhevmType.euint64, handle, await wrapper.getAddress(), bob);

        expect(decrypted).to.equal(wrapAmount);
      });

      it("should revert on insufficient allowance", async function () {
        await expect(wrapper.connect(alice).wrap(alice.address, wrapAmount + 1n)).to.be.reverted;
      });
    });
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  //                        HETU PAYMENT GATEWAY
  // ═══════════════════════════════════════════════════════════════════════════

  describe("HetuPaymentGateway", function () {
    beforeEach(async function () {
      const wrapperAddr = await wrapper.getAddress();
    ...
    describe("Send Payment", function () {
      ...
      it("should send encrypted payment", async function () {
        ...
        const tx = await gateway.connect(alice).send(bob.address, encrypted.handles[0], encrypted.inputProof);
    ...
    describe("Multi-Send", function () {
      ...
      it("should revert with more than 10 recipients", async function () {
        ...
        await expect(gateway.connect(alice).multiSend(recipients, handles, proofs)).to.be.revertedWith(
          "1-10 recipients",
        );
      });
    });
    ...
    describe("Payment Requests", function () {
      ...
      it("should create payment request", async function () {
        ...
        const tx = await gateway.connect(alice).createRequest(encrypted.handles[0], encrypted.inputProof, 86400);
        const receipt = await tx.wait();
      ...
      it("should fulfill payment request", async function () {
        ...
        const createTx = await gateway
          .connect(alice)
          .createRequest(encrypted1.handles[0], encrypted1.inputProof, 86400);
        ...
        const fulfillTx = await gateway
          .connect(bob)
          .fulfillRequest(requestId, encrypted2.handles[0], encrypted2.inputProof);
      ...
      it("should cancel payment request", async function () {
        ...
        const createTx = await gateway.connect(alice).createRequest(encrypted.handles[0], encrypted.inputProof, 86400);
        const createReceipt = await createTx.wait();
    ...
    describe("Subscriptions", function () {
      ...
      it("should create subscription", async function () {
        ...
        const tx = await gateway
          .connect(alice)
          .createSubscription(bob.address, encrypted.handles[0], encrypted.inputProof, interval);
        const receipt = await tx.wait();
      ...
      it("should revert subscription with interval less than 1 day", async function () {
        ...
        await expect(
          gateway.connect(alice).createSubscription(bob.address, encrypted.handles[0], encrypted.inputProof, 3600),
    ...
    describe("Refunds", function () {
      ...
      it("should revert double refund", async function () {
        ...
        await expect(gateway.connect(bob).refund(paymentId)).to.be.revertedWith("Already refunded");
      });

      it("should revert refund by non-recipient", async function () {
        await expect(gateway.connect(alice).refund(paymentId)).to.be.revertedWith("Not recipient");
      });
    });
    ...
    describe("View Functions", function () {
      ...
      it("should return subscription info", async function () {
        ...
        const tx = await gateway
          .connect(alice)
          .createSubscription(bob.address, encrypted.handles[0], encrypted.inputProof, 86400);
        const receipt = await tx.wait();
    ...
    describe("Admin Functions", function () {
      it("should allow owner to change default token", async function () {
        ...
        const newWrapper = await NewWrapper.deploy(await usdc.getAddress(), "New cUSDC", "ncUSDC", "");
      ...
      it("should revert non-owner token change", async function () {
        await expect(gateway.connect(alice).setDefaultToken(ethers.ZeroAddress)).to.be.revertedWith("Not owner");
      });
      ...
      it("should revert ownership transfer to zero address", async function () {
        await expect(gateway.connect(owner).transferOwnership(ethers.ZeroAddress)).to.be.revertedWith("Invalid owner");
      });
    });
  });
});
