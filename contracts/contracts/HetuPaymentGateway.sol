...
/// @title HetuPaymentGateway - Privacy-First Payment Gateway
/// @dev Hetu (அருவி) means "waterfall" in Tamil - privacy flows freely

contract HetuPaymentGateway is ZamaEthereumConfig {
    ...
    function _send(
        ...
        euint64 transferred = IERC7984(token).confidentialTransferFrom(sender, recipient, amount);
        ...
        // Generate payment ID
        paymentId = keccak256(
            abi.encodePacked(sender, recipient, block.timestamp, block.number, paymentCount[sender]++)
        );
    ...
    function _createRequest(
        ...
        requestId = keccak256(abi.encodePacked(requester, block.timestamp, block.number, requestCount[requester]++));
    ...
    function createSubscription(
        ...
        subscriptionId = keccak256(
            abi.encodePacked(msg.sender, recipient, block.timestamp, subscriptionCount[msg.sender]++)
        );
    ...
    function executeSubscription(bytes32 subscriptionId) external returns (bytes32 paymentId) {
        ...
        euint64 transferred = IERC7984(sub.token).confidentialTransferFrom(sub.subscriber, sub.recipient, sub.amount);
        FHE.allowThis(transferred);

        ebool hasTransferred = FHE.gt(transferred, FHE.asEuint64(0));

        // Generate payment ID
        paymentId = keccak256(abi.encodePacked(sub.subscriber, sub.recipient, block.timestamp, block.number));
    ...
    function refund(bytes32 paymentId) external {
        ...
        IERC7984(payment.token).confidentialTransferFrom(msg.sender, payment.sender, payment.amount);
    ...
    function getPaymentInfo(
        bytes32 paymentId
    ) external view returns (address sender, address recipient, address token, uint256 timestamp, bool isRefunded) {
    ...
    function getRequestInfo(
        bytes32 requestId
    ) external view returns (address requester, address token, uint256 createdAt, uint256 expiresAt, bool fulfilled) {
    ...
    function getSubscriptionInfo(
        ...
        external
        view
        returns (address subscriber, address recipient, uint256 interval, uint256 nextPayment, bool active)
    {
